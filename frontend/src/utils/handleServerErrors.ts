...
export default function handleServerErrors(
  ...
  return new Promise((resolve, reject) => {
    // Local mode: bypass login redirect
    if ((response.status === 401 || response.status === 403) && !useCloud) {
      console.warn("Bypassed auth redirection for local mode:", response.status);
      return reject(new Error("Session expired or unauthorized. Ensure your backend is running."));
    }
    if ((response.status === 401 || response.status === 403) && useCloud) {
